...
<?php $data = $user->get_jenisujiansusulanid($_GET['id'])?>
<?php $datajadwalujian = $user->get_datajadwalujianmatakuliahid($data['idjadwalujian']) ?>
<?php $datamhs = $user->get_datamahasiswa(session::get('username')) ?>
<?php $tampil = $datajadwalujian['soaltampil']; ?>

<?php 
$datanilai = $user->ck_dbnilaiperus($datamhs['NIM'],$data['id']);
...
date_default_timezone_set("Asia/Bangkok");
    ...
    if ($tanggalsekarang>$waktuselesai) {
        if ($datanilai['nilai'] == '') {
            ...
            echo "<script>location='index.php?halaman=uss';</script>";
            die();
        }
    ...
    if (input::get('simpanjawaban')==='Simpan Jawaban') {
        ...
        for ($i=0; $i < $tampil ; $i++) { 
            ...
            $input = array(
                ...
                'idnilaimtk'        => $datanilai['id'],
            );
            $user->simpan_datajawabanmhs($input,input::get('ids'.$i));
        }
        $user->hitungnilaimhs('ujiansusulan',$datanilai['id']);
        echo "<script>location='index.php?halaman=pujs&t=$_GET[t]&id=$_GET[id]';</script>";
    }
                        ...
                        <div class="card">
                            <div class="header colorblue" style="padding: 20px;">
                                <h3 class="title" style="color: white">
                                    Ujian Susulan 
                                    <?php 
                                        $dtmtk = $user->get_datamatakuliah($datajadwalujian['idmatakuliah']);
                                        echo $dtmtk['namamatakuliah'] ;
                                        ?> 
                                    </h3>
                            ...
                            <div class="content">
                                <form method="post">
                                    <div class="row">
                                            ...
                                            <?php 
                                                if (empty($datanilai)) {
                                                    $datasoal = $user->acaksoal('ujiansemester',$datajadwalujian['id']);
                                                    ...
                                                    if (!empty($datasoal)) {
                                                        ...
                                                        $fields = array(
                                                            'id'                    =>  $idnilai,
                                                            'NIM'                   =>  $datamhs['NIM'],
                                                            'idjadwalujiansusulan'  =>  $data['id'],
                                                            'idmatakuliah'          =>  $datajadwalujian['idmatakuliah'],
                                                            );

                                                        $user->tambah_datanilaiujiansusulan($fields);
                                                    ...
                                                    for ($i=0; $i < $tampil; $i++) {
                                                        ...
                                                        foreach ($datapg as $pg) {
                                                                if ($datasoal[$i]['pilihanjawaban'] === $pg) {
                                                                    ...
                                                                    echo '<input type="radio" class="pg" name="pg'.$i.'" data-id="'.$i.'" value="'.$pg.'" style="margin : 10px">'.$pg.'<input type="hidden" name="ids'.$i.'" value="'.$datasoal[$i]['soal'].'"><br>';
                                                                }
                                                        }
